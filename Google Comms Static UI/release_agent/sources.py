...
_RELEASE_NOTES_PAGE_URL = "https://docs.cloud.google.com/release-notes"
# Google's release-notes page only keeps ~60 days of per-date anchors live;
# older anchors 404, so this mirrors the same rolling cutoff sql/03_clean_dataset
# already applies at the SQL layer, just computed here in Python instead —
# the serving `notes` table (SETTINGS.table_fqn) never carries a source_url
# column, so every note's link is derived client-side from its own date.
_SOURCE_URL_ANCHOR_WINDOW_DAYS = 60


def release_note_source_url(published_at: str | date | datetime) -> str:
    """Builds the deep link into Google's release-notes page for a given date.

    Mirrors the anchor scheme in sql/01_create_release_notes_table.sql
    (`FORMAT_DATE('%B_%d_%Y', ...)`) so a note's link always points at the
    exact day it was published, falling back to the general release-notes
    page once that anchor is old enough to have rolled off Google's page.
    """
    if isinstance(published_at, datetime):
        parsed = published_at.date()
    elif isinstance(published_at, date):
        parsed = published_at
    else:
        parsed = date.fromisoformat(str(published_at)[:10])

    if (date.today() - parsed).days > _SOURCE_URL_ANCHOR_WINDOW_DAYS:
        return f"{_RELEASE_NOTES_PAGE_URL}/all"
    return f"{_RELEASE_NOTES_PAGE_URL}#{parsed.strftime('%B_%d_%Y')}"
...
def _rows_to_notes(rows) -> list[ReleaseNote]:
    ...
    return [
        ReleaseNote(
            ...
            source_url=release_note_source_url(row.published_at),
        )
        for row in rows
    ]
            ...
            "summary": [
                {
                    ...
                    "source_url": release_note_source_url(row.latest),
                }
                for row in rows
            ],
